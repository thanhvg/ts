...
Sync with upstream, removed javscript layer from dependency
...
If you want to use ts-formatter for formatting run:

#+BEGIN_SRC shell
  npm install -g ts-formatter
#+END_SRC
...
** Choosing a backend
To choose a default backend set the layer variable =ts-backend=:

#+BEGIN_SRC elisp
  (setq-default dotspacemacs-configuration-layers '(
    (typescript :variables ts-backend 'tide)))
#+END_SRC
...
#+BEGIN_SRC elisp
  ...
  ((typescript-mode (ts-backend . lsp)))
#+END_SRC
